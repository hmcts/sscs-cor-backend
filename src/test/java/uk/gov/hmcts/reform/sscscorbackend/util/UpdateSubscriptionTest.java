...
public class UpdateSubscriptionTest {
    ...
    private final SscsCaseData existingAppointeeCase = SscsCaseData.builder()
        ...
        .subscriptions(Subscriptions.builder()
            ...
            .appointeeSubscription(Subscription.builder()
                ...
                .subscribeSms("Yes").mobile("0777").email("appointee@email.com").subscribeEmail("Yes").build())
            .build()
        ).build();

    @Test
    public void givenLastLoggedIntoMya_shouldUpdateAppointeeAndRep() {
        String lastLoggedIntoMya = "2020-01-24T00:00:00+01:00";
        final Subscription expectedRepSubscription = Subscription.builder().email("harry.potter@mail.com")
                .lastLoggedIntoMya(lastLoggedIntoMya).subscribeSms("Yes").mobile("0777")
                .subscribeEmail("Yes").build();

        final Subscription expectedAppointeeSubscription = Subscription.builder().email("appointee@email.com")
            .lastLoggedIntoMya(lastLoggedIntoMya).wantSmsNotifications("Yes").subscribeSms("Yes").mobile("0777")
                .subscribeEmail("Yes").build();
        ...
        UpdateSubscription.updateSubscription(existingAppointeeCase, appointeeSubscriptionUpdate, lastLoggedIntoMya);
